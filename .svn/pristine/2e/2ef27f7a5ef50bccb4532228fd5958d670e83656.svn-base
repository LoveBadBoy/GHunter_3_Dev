...
- (IBAction)determineClickBtn:(id)sender {
    ...
    if (self.shareGoldTotalNumTF.text.length == 0 && self.shareGoldTotalNumTF.text.integerValue == 0) {
    ...
    if (qzone + qFriend + wxCircle + wxFriend + weibo == 0) {
        [ProgressHUD show:@"至少一个分享平台金币数大于0"];
        return;
    }
