    ...
    if (self) {
        ...
        for (int i = 0; i <sectionNum; i++) {
            ...
            if (i<sectionNum && i != 0) {
                ...
                lineView.backgroundColor = [UIColor redColor];//中间的竖线的颜色
...
//        self.mTableBaseView = [[UIView alloc] initWithFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y + self.frame.size.height , self.frame.size.width, self.mSuperView.frame.size.height - self.frame.origin.y - self.frame.size.height)];
    ...
    CGRect rect = self.mTableView.frame;
    rect.origin.x = 0;//修改下来的tableView的位置
    rect.size.width = 320;
    rect.size.height = 0;
    self.mTableView.frame = rect;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(25, 5, 80, 30)];
    label.text = [self.dropDownDataSource titleInSection:currentExtendSection index:indexPath.row];

    [cell.contentView addSubview:label];
    label.font = [UIFont systemFontOfSize:14];
    
    imgarr = [NSArray arrayWithObjects:@"mylist1",@"mylist2",@"mylist3",@"mylist4", nil];
    UIImageView *image = [[UIImageView alloc]initWithFrame:CGRectMake(5, 5, 20, 20)];
    image.image =[UIImage imageNamed:[imgarr objectAtIndex:indexPath.row]];
