...
- (IBAction)address:(UIButton *)sender {
    self.backView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, mainScreenWidth, mainScreenheight)];
...
- (void)configureCustomPikerView {
    ...
    customPikerView.backgroundColor = [UIColor whiteColor];
    customPikerView.frame= CGRectMake(0, self.backView.frame.size.height-200, mainScreenWidth, 200);
    ...
    [self.backView addSubview:customPikerView];
...
- (void)viewDidLoad
{
    ...
    userinfoFrame.size.width = self.view.frame.size.width;
    self.userInfo.frame = userinfoFrame;
    self.scrollView.frame = CGRectMake(0, 64, self.view.frame.size.width, mainScreenheight - 20 - 44);
    self.scrollView.contentSize = CGSizeMake(self.view.frame.size.width, self.userInfo.frame.size.height);
